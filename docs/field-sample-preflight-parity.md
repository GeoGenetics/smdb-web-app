...
| Field-sample identity and parent relationships | Validates field-sample ID uppercase and syntax only. | `field_sample_id_validate_alpha2_only()` also validates country-code and sample-year agreement; the primary key, self-reference foreign key, and cycle-prevention trigger validate current table state. | Keep country/year matching, duplicate IDs, and ancestry database-only until an explicitly scoped ID/parent validation slice is designed. |
